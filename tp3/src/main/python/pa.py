...
def study(name: str):
    ...
    return {
        'length': study(LENGTHS, (r"$L$ $(m)$", r"$‹P›$ $(N/m)$")),
        'area': study(AREA, (r"$A$ $(m^2)$", r"$‹P›$ $(N/m)$"), False, (True, False)),
        'areant': study(AREANT, (r"$A^{-1}$ $(1/m^2)$", r"$‹P›$ $(N/m)$"), True),
        'pa': study(PA, (r"$L$ $(m)$", r"$‹P›A$ $(Nm)$"))
    }.get(name)
...
if __name__ == '__main__':
    ...
    if FIT:
        ...
        _, _, M, _ = fitter(np.array(X), np.array(Y), LINEAR(0), 0, 0.027)
